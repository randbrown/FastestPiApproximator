...
namespace FastestPiApproximator
{
    class Program
    {
        ...
        static int numTerms = 1000000;

        //DO NOT TOUCH! Used for internal calculations. Program will not work if you tamper.
        static mpz_t arb = mpz_t.Power(10, digits);

        //DO NOT TOUCH! Used for internal calculations. Program will not work if you tamper.
        // static mpz_t numThreadsExp = mpz_t.Pow(16, numThreads);
        ...
        static mpz_t PSum_Arctan_2Term(int l, int id)
        {
            ...
            //List<mpz_t> totals = new List<mpz_t>();
                ...
                k += numThreads)
            {
                ...
                //The original method
                //total += (((arb) / (mpz_t.Pow(16, k * numThreads)) * (4 * (arb) / (8 * k + 1) - 2 * (arb) / (8 * k + 4) - (arb) / (8 * k + 5) - (arb) / (8 * k + 6))) / arb;
            ...
            //return total;
            //mpz_t totalFromString = mpz_t.Parse(totalToReturn);
